...
def build_phase_inputs():
    ...
    for phase_key in PHASE_KEYS:
        ...
        phase_inputs.append({
            ...
            "samples_per_sku": defaults["samples_per_sku"],
        })
    return phase_inputs
...
def build_weekly_plan(
    ...
    videos_per_sample = prod_df["Videos / Sample"].to_numpy()
    clicks_per_video = prod_df["Clicks / Video"].to_numpy()
    click_to_order_rate = prod_df["Click-to-order Rate"].to_numpy()
    ...
    for phase in phase_inputs:
        for week_idx in range(int(weeks_in_phase)):
            global_week += 1
            samples_p = np.full(len(prod_df), float(phase["samples_per_sku"]))
...
with st.sidebar:
    ...
    for i, phase in enumerate(phase_inputs):
        ...
        phase["ads_take_rate"] = st.number_input(
            f"{T['ads_rate']} (%) - {phase['name']}",
            min_value=0.0,
            max_value=30.0,
            value=float(phase["ads_take_rate"] * 100.0),
            step=1.0,
            key=f"ads_{i}",
            help=T["ads_rate_help"],
        ) / 100.0

        phase["samples_per_sku"] = st.number_input(
            f"{T['samples_per_week']} / SKU - {phase['name']}",
            min_value=0,
            value=int(phase["samples_per_sku"]),
            step=1,
            key=f"samples_per_sku_{i}",
        )
